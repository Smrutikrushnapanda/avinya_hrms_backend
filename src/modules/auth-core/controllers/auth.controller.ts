...
export class AuthController {
  ...
  @Get('profile')
  @ApiOperation({ summary: 'Get logged-in user profile with isApprover flag' })
  @ApiResponse({
    status: 200,
    description: 'Returns enhanced user profile with isApprover boolean',
    schema: {
      example: {
  ...
  async getProfile(@GetUser() user: any) {
    return this.authService.getEnhancedProfile(user);
  }
}
